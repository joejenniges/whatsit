...
// audioSetLabels contains all 527 AudioSet class labels in index order.
// These must match the CED model's output neuron indices exactly.
//
// Source: https://github.com/qiuqiangkong/audioset_tagging_cnn/master/metadata/class_labels_indices.csv
// The labels are stable and rarely change.
var audioSetLabels = []string{
	"Speech",                                    // 0
...
// LoadAudioSetLabels returns the 527 AudioSet class labels in index order.
// The label at index N corresponds to output neuron N of the CED model.
func LoadAudioSetLabels() []string {
	return audioSetLabels
}

// --- Label category helpers ---
// These check if a label belongs to a specific category relevant to
// radio classification (speech vs music vs singing).

var speechLabels = map[string]bool{
	"Speech":                              true,
	...
	"Whispering":                          true,
	"Shout":                               true,
	"Speech synthesizer":                  true,
	"Hubbub, speech noise, speech babble": true,
}

var singingLabels = map[string]bool{
	"Singing":          true,
	"Male singing":     true,
	"Female singing":   true,
	"Child singing":    true,
	...
	"Chant":            true,
	"Rapping":          true,
	"Synthetic singing": true,
	"Vocal music":      true,
	"A capella":        true,
	"Opera":            true,
}

var musicTopLabels = map[string]bool{
	"Music":              true,
...
var genreLabels = map[string]bool{
	...
	"Trance music":           true,
	"Electronica":            true,
	"Electronic dance music": true,
	"Dance music":            true,
	"Independent music":      true,
	"Christian music":        true,
	"Middle Eastern music":   true,
	"Music of Latin America": true,
	"Music of Africa":        true,
	"Music of Asia":          true,
	"Carnatic music":         true,
	"Music of Bollywood":     true,
	"Traditional music":      true,
}

func isSpeechLabel(label string) bool  { return speechLabels[label] }
func isSingingLabel(label string) bool { return singingLabels[label] }
func isMusicLabel(label string) bool   { return musicTopLabels[label] || genreLabels[label] }
func isGenreLabel(label string) bool   { return genreLabels[label] }
